            ...
            if PlayerJob.name == "dot" then
                            ...
                            if IsPedInAnyVehicle(PlayerPedId(), false) then
                                if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId() then
                                    if isTruckerVehicle(GetVehiclePedIsIn(PlayerPedId(), false)) then
                                        ...
                                        TriggerServerEvent('qb-dot:server:DoBail', false)
                                    else
                ...
                if #(pos - vector3(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z)) < 4.5 then
                    if #(pos - vector3(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z)) < 1.5 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            if JobsDone > 0 then
                                TriggerServerEvent("qb-dot:server:01101110", JobsDone)
                                JobsDone = 0
                                    ...
                                    if IsControlJustReleased(0, 38) then
                                        ...
                                        QBCore.Functions.Progressbar("work_dropbox", "Deliver Box Of Products", 2000, false, true, {
                                            ...
                                            if currentCount == CurrentLocation.dropcount then
                                                ...
                                                if chance < 26 then
                                                    TriggerServerEvent('qb-dot:server:nano')
                                                end
                                                if CurrentBlip ~= nil then
                                                    RemoveBlip(CurrentBlip)
						                            ClearAllBlipRoutes()
...
function getNewLocation()
    ...
    if location ~= 0 then
        ...
        QBCore.Functions.Notify("You Finished All Your Jobs .. Time For Your Payslip!")
        if CurrentBlip ~= nil then
            RemoveBlip(CurrentBlip)
	    ClearAllBlipRoutes()
            CurrentBlip = nil
...
function TakeOutVehicle(vehicleInfo)
    TriggerServerEvent('qb-dot:server:DoBail', true, vehicleInfo)
...
RegisterNetEvent('qb-dot:client:SpawnVehicle')
AddEventHandler('qb-dot:client:SpawnVehicle', function()
