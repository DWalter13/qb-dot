...
RegisterServerEvent('qb-dot:server:DoBail')
AddEventHandler('qb-dot:server:DoBail', function(bool, vehInfo)
    local src = source
    ...
    if bool then
        if Player.PlayerData.money.cash >= Config.BailPrice then
            ...
            TriggerClientEvent('qb-dot:client:SpawnVehicle', src, vehInfo)
...
RegisterNetEvent('qb-dot:server:01101110')
AddEventHandler('qb-dot:server:01101110', function(drops)
    local src = source 
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('qb-dot:server:nano')
AddEventHandler('qb-dot:server:nano', function()
